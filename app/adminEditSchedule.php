...
try {
  ...
  $index = 0;
  if ($event != []) {
    $sql = "INSERT INTO schedule ( name, date, client, agenda, start_time, end_time, total_time, admin_total_time, staff_hour_salary, staff_day_salary, staff_expense, admin_hour_salary, admin_day_salary , admin_expense ) VALUES ";
    ...
    foreach ($event as $values) {
      ...
      $sql_values = "( '{$values['name']}', '{$values['date']}', '{$values['client']}', '{$values['agenda']}', '{$values['start_time']}', '{$values['end_time']}', '{$values['total_time']}', '{$values['admin_total_time']}',
