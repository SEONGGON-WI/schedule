    ...
    if (password_verify($password, $hashedPassword)) {
      ...
      if ($event != []) {
        foreach ($event as $values) {
          $sql = "UPDATE schedule SET start_time = '{$values['start_time']}', end_time ='{$values['end_time']}', total_time = '{$values['total_time']}', admin_total_time = '{$values['admin_total_time']}', staff_hour_salary = '{$values['staff_hour_salary']}', staff_day_salary = '{$values['staff_day_salary']}', staff_expense = '{$values['staff_expense']}' WHERE _id = '{$values['_id']}' AND name = '{$values['name']}' AND date = '{$values['date']}'";
